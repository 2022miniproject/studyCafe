...
int updateFood(struct food_struct *p[], int count); // 5-4. 주문한 음식 수정하기
void order(struct food_struct *p[], int count);		// 5-5. 음식 주문 넣기 (음식 주문 확정하기)
	...
	while (1)
	{
		...
		else if (menu == 5)
		{
			...
			while (1)
			{
				...
				{
					num = plusfood(Order, num);
				}
				else if (select == 3)
				{
					num = deletefood(Order, num);
				}
				else if (select == 4)
				{
					num = updateFood(Order, num);
				}
				else if (select == 5)
				{
					order(Order, num);
